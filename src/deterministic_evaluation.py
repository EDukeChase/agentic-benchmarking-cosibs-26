...
# Containers/CI have no display; Agg renders straight to a file.
matplotlib.use("Agg")
...
# Stack correct predictions (green) below incorrect ones (orange/red) per
# model, so right-vs-wrong is visible at a glance across the whole run.
def plot_diagnosis_accuracy(run_dir: Path, predictions: dict[str, pd.DataFrame]) -> Path | None:
   model_names = []
   ...
   for model_name in sorted(predictions.keys()):
       table = predictions[model_name]
       true_diagnosis = table["true_diagnosis"].astype(bool)
       generated_diagnosis = table["generated_diagnosis"].astype(bool)
       model_names.append(model_name)
       true_positive_counts.append(int((true_diagnosis & generated_diagnosis).sum()))
       true_negative_counts.append(int((~true_diagnosis & ~generated_diagnosis).sum()))
       false_positive_counts.append(int((~true_diagnosis & generated_diagnosis).sum()))
       false_negative_counts.append(int((true_diagnosis & ~generated_diagnosis).sum()))
...
def evaluate_run(run_id: str, task: BenchmarkTaskConfig) -> dict[str, dict[str, float]]:
   ...
   plot_diagnosis_accuracy(run_dir, predictions)
   return results
