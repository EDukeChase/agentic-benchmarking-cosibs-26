...
from generated_code.c69c2d08.logistic_regression.model import LogisticRegressionModel
...
def benchmark():
    X, y = _build_feature_matrix_ehr()
    split = int(len(X) * 0.7)
    X_train, X_test = X.iloc[:split], X.iloc[split:]
    y_train, y_test = y[:split], y[split:]
    vec = TfidfVectorizer(max_features=2000, ngram_range=(1, 2))
    X_train_vec = vec.fit_transform(X_train['text']).toarray()
    X_test_vec = vec.transform(X_test['text']).toarray()
    model = LogisticRegressionModel(max_iter=200)
    model.fit(X_train_vec, y_train)
    score = model.predict_proba(X_test_vec)[:, 1]
